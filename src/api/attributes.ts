...
export type GetAttributeCommentsPayload = {
  view?: AttributesViewType
  offset?: number
  archetypeId: ArchetypeKey
  lang?: string
...
export async function getAttributeComments(
  params: GetAttributeCommentsPayload,
): Promise<AttributeCommentsData> {
  const response = await apiClient.get<ApiResponse<AttributeCommentsData>>("/attributes/comments", {
    params,
  })
  return response.data.data
}

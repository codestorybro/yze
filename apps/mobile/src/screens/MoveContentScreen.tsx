...
export function MoveContentScreen({ currentPlaceId, entityId, kind }: MoveContentScreenProps) {
  ...
  useEffect(() => {
    ...
    void fetchLevel(null).then((result) => {
      if (sequence !== requestSequence.current) return
      if (result.kind === "ok") setPlaces(result.data)
      else setLoadError(apiFailureMessage(result))
      setLoading(false)
    })
    return () => {
  ...
  const browse = async (place: PlaceSummary) => {
    await transitionTo([...path, place])
  }
  ...
  const moveTo = async (destinationId: string | null) => {
    ...
    const result =
      kind === "item"
        ? destinationId
          ? await moveItem(entityId, destinationId)
          : null
        : await movePlace(entityId, destinationId)
    setSubmitting(false)

    if (!result) {
      setMessage("Select a destination Place.")
      return
    }

    if (result.kind === "ok") {
      showToast(kind === "item" ? "Item moved" : "Place moved")
      void notifySuccess()
      router.dismissTo((destinationId ? `/places/${destinationId}` : "/places") as Href)
    } else {
  ...
  return (
    <SheetList
      ...
      ListHeaderComponent={
        <View style={themed($page)}>
          <FeatureHeader
            ...
            subtitle="Browse the hierarchy, select a Place, then confirm the destination."
          />
          ...
          {kind === "place" && path.length === 0 ? (
            <Button
              preset="secondary"
              disabled={!currentPlaceId || submitting}
      ...
      ListEmptyComponent={
        loading ? null : (
          <View style={themed($empty)}>
            <Text style={themed($muted)} text="There are no child Places at this level." />
          </View>
        )
      }
      ListFooterComponent={
          ...
          {selected ? (
            <View style={themed($confirmation)}>
              ...
              <Button
                preset="primary"
                disabled={submitting}
                onPress={() => void moveTo(selected.id)}
                text={submitting ? "Moving…" : "Move here"}
              />
            </View>
          ) : null}
          ...
          {loadError ? (
            <View style={themed($message)}>
              ...
              <Button
                preset="ghost"
                disabled={loading}
                onPress={() => void transitionTo(path)}
                text="Try again"
              />
            </View>
          ) : null}
      ...
      renderItem={({ item: place }) => {
        const selectionUnavailable = place.id === currentPlaceId || place.id === entityId
        const browseUnavailable = kind === "place" && place.id === entityId
        ...
        return (
          <View style={[themed($row), isSelected && themed($selectedRow)]}>
            ...
            {place.childPlaceCount > 0 && !browseUnavailable ? (
              <Pressable
                accessibilityLabel={`Browse inside ${place.name}`}
                accessibilityRole="button"
                disabled={loading}
                hitSlop={8}
                onPress={() => void browse(place)}
                style={themed($browse)}
              >
                <SymbolView
                  name={{
